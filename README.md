...
github.com/yudeguang/ratelimit底层用环形队列来存储用户访问数据，拥有较高的性能,每秒约能处理250W次(但也有缺点，会耗用一定的内存，不宜设置时间跨度过大的规则，比如每周，每个月允许访问多少次)。作为对比，与用redis的相关数据结构来实现用户访问控制相比，其用法相对简单。 
